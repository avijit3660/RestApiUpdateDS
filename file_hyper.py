...
employee_table = TableDefinition(
    table_name="Employee",
    # table_name=("Extract","Employee"),
    # Since the table name is not prefixed with an explicit schema name,
    # the table will reside in the default "public" namespace.
    columns=[
        TableDefinition.Column("Emp Name", SqlType.text(), NOT_NULLABLE),
        TableDefinition.Column("Salary", SqlType.big_int(), NOT_NULLABLE),
        TableDefinition.Column("Active", SqlType.text(), NOT_NULLABLE)
    ]
)
...
def run_create_hyper_file_from_csv():
    ...
    """
    print("Loading data from CSV into table in new Hyper file")

    # path_to_database = Path("employee.hyper")
    path_to_database = Path("new_data.hyper")
    ...
    with HyperProcess(telemetry=Telemetry.SEND_USAGE_DATA_TO_TABLEAU, parameters=process_parameters) as hyper:
        ...
        with Connection(endpoint=hyper.endpoint,
            ...
            connection.catalog.create_table(table_definition=employee_table)
            ...
            path_to_csv = str(Path(__file__).parent / "new_data.csv")
            ...
            count_in_customer_table = connection.execute_command(
                command=f"COPY {employee_table.table_name} from {escape_string_literal(path_to_csv)} with "
                        f"(format csv, NULL 'NULL', delimiter ',', header)")

            print(f"The number of rows in table {employee_table.table_name} is {count_in_customer_table}.")
